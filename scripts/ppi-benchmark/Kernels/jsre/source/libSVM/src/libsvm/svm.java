...
public class svm {
	...
	//Attention, method is also used in: org.itc.irst.tcc.sre.util;
	public static double svm_predict(svm_model model, svm_node[] x)
	{
		...
		{	
			...
			for(i=0;i<nr_class;i++)
				vote[i] = 0;
			int pos=0;
			double phil=- Double.MIN_VALUE;
			for(i=0;i<nr_class;i++)
				for(int j=i+1;j<nr_class;j++)
				{
					phil=dec_values[pos]; 	//Distance to the hyperplane
//					System.out.println("i: " +i +", j:" +j +", dev_values " +dec_values[pos]);
					if(dec_values[pos++] > 0)
						++vote[i];
					else
						++vote[j];
			...
			for(i=1;i<nr_class;i++)
				if(vote[i] > vote[vote_max_idx])
					vote_max_idx = i;
//			System.out.println("class:" +model.label[vote_max_idx]);
...
//			return 1;
	...
	public static double [] svm_predict2(svm_model model, svm_node[] x)
	{
		if(model.param.svm_type == svm_parameter.ONE_CLASS ||
		   model.param.svm_type == svm_parameter.EPSILON_SVR ||
		   model.param.svm_type == svm_parameter.NU_SVR)
...
//			System.out.println(phil);
			...
			ret[0]= model.label[vote_max_idx];
			ret[1]= phil;
//			return model.label[vote_max_idx];
			return ret;
