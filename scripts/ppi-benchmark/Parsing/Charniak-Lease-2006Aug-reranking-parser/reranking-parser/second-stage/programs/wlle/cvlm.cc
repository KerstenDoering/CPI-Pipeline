...
//
const char usage[] =
"cvlm version of 11th April 2005\n"
"\n"
...
"\n"
"Usage: cvlm [-help] [-debug debug_level] [-c0 c0] [-p p] [-r r] [-s s] [-cv] \n"
"              [-l ltype] [-Pyx_factor f] [-Px_propto_g] \n"
...
struct Estimator1 {
  ...
  Estimator1(corpus_type* train, corpus_type* eval, corpus_type* eval2,
	     loss_type ltype, double c0, double p, double r, double s, bool cv, 
	     bool opt_fscore = true, const char* weightsfile = NULL) 
    : train(train), nx(train->nfeatures), eval(eval), eval2(eval2),
      ltype(ltype), c0(c0), p(p), r(r), s(s), cv(cv), 
  ...
  if (evalfile2 != NULL) {
    ...
    if (debug_level >= 10)
      std::cerr << "# read evalfile2 = " << evalfile2 
		...
		<< std::endl;
    int nxe = evaldata2->nfeatures;
    assert(nxe <= nx);
  }

  Estimator1 e(traindata, evaldata, evaldata2, ltype, c0, p, r, s, cv, true,
	       tao_env.get_cstr_option("-o"));
