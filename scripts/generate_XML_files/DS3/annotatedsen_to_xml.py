...
if __name__=="__main__":
    ...
    parser.add_option("-o", "--output", dest="o", help='name of the output file',default="DS3.xml")
    ...
    outfile.write("<corpus source=\"DS3\">"+"\n")
...
#        print TF_int_list
        ...
        if curr_pmid == pre_pmid :
            ...
            outfile.write("        <sentence id=\"DS3.d"+str(doc_num-1)+".s"+str(sent_num)+"\" origId=\""+str(pmid_sent_num)+"\" text="+quoteattr(pur_sent_text)+">"+"\n")

            # build entity tags according to the list identified tags from the CSV file (entity_list)
            for i in range(0,len(entity_list)) :
                outfile.write("            <entity id=\"DS3.d"+str(doc_num-1)+".s"+str(sent_num)+".e"+str(i)+"\" origId=\""+entity_list[i][1]+"\" charOffset=\""+entity_list[i][3]+"\" type=\""+entity_list[i][0]+"\" text="+quoteattr(entity_list[i][2])+"/>"+"\n")

            # insert types of interaction for each pair of entities
            # get the index of the synonym interactions in entity_list
            origId = "DS3.d"+str(doc_num-1)+".s"+str(sent_num)
        ...
        # if the current PubMed ID changed in comparison to the last parsed sentences
            ...
            outfile.write("    <document id=\"DS3.d"+str(doc_num)+"\" origId=\""+str(curr_pmid)+"\">"+"\n")
            # replace squared brackets ([,]) with round brackets
            pur_sent_text = pur_sent_text.replace("[","(").replace("]",")")
            outfile.write("        <sentence id=\"DS3.d"+str(doc_num)+".s"+str(sent_num)+"\" origId=\""+str(pmid_sent_num)+"\" text="+quoteattr(pur_sent_text)+">"+"\n")
            # now have to make entity tags according to entity_list data.
            for i in range(0,len(entity_list)) :
                outfile.write("            <entity id=\"DS3.d"+str(doc_num)+".s"+str(sent_num)+".e"+str(i)+"\" origId=\""+entity_list[i][1]+"\" charOffset=\""+entity_list[i][3]+"\" type=\""+entity_list[i][0]+"\" text="+quoteattr(entity_list[i][2])+"/>"+"\n")
            # build entity tags
            origId = "DS3.d"+str(doc_num)+".s"+str(sent_num)
            for int_id in range(len(interaction_list)) :
                for ent_id in range(len(entity_list)):
                    if interaction_list[int_id][0] in entity_list[ent_id]:
                        break
