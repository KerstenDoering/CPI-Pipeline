...
#    In case of the shallow linguistic kernel approach, only the syntactic tree parse is needed, which will be generated within the first make-experiment step. To compare this method with the all-paths graph kernel approach, the cross-validation files are copied from the all-path graph kernel folder. Alternatively, these files can be copied from their original directories as used in the script APG_pipeline_DS2.sh.
...
python modify_2.py
python rename_files_splitting_DS2_again.py
# change into directory Experiments to start the calculation and the upload process
cd ../../Experiments
echo "run SL pipeline ..."
make experiment Corpora="DS2" Kernel="SL" expType="CV"
echo "uploading results to PostgreSQL ..."
make output2db Corpora="DS2" Kernel="SL" expType="CV"
cd ..
cd Database
